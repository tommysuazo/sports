...
use App\Repositories\WnbaTeamStatRepository;
use Illuminate\Support\Collection;

class WnbaTeamService
{
    ...
    public function __construct(
        protected WnbaTeamStatRepository $wnbaTeamStatRepository,
    ) {
    }

    public function getTeamStats(WnbaTeam $team): WnbaTeam
    {
        return $this->wnbaTeamStatRepository->loadWithStats($team);
    }

    public function getTeamAverageStats(WnbaTeam $team): array
    {
        return [
            'team' => [
                'id' => $team->id,
            ...
            'games_with_stats' => $this->wnbaTeamStatRepository->countStats($team),
            'averages' => $this->wnbaTeamStatRepository->getAverageStats($team),
        ];
    }

    public function getTeamsAverageStats(): array
    {
        $teams = WnbaTeam::orderBy('name')->get(['id', 'name', 'short_name', 'city']);
        $aggregated = $this->wnbaTeamStatRepository->getAverageStatsForAllTeams();

        $teamsData = $teams->mapWithKeys(function (WnbaTeam $team) use ($aggregated) {
    ...
    public function getTeamsRecentPerformance(int $games = 7): array
    {
        ...
        $teamsData = $teams->map(function (WnbaTeam $team) use ($games) {
            $recentStats = $this->wnbaTeamStatRepository->getRecentStatsWithGameData($team, $games);
    ...
    protected function resolveOpponentPoints(WnbaTeamStat $stat, int $teamId): ?float
    {
        $game = $stat->game;
        ...
        $opponentStat = $game->stats
            ->first(fn (WnbaTeamStat $gameStat) => (int) $gameStat->team_id !== $teamId);
