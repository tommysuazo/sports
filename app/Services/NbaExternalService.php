...
class NbaExternalService
{
    ...
    public static function getGameDayUrl(Carbon $date)
    {
        $response = Http::withHeaders(self::HEADERS)
            ->get(self::BASE_URL . '/stats/scoreboardv3?DayOffset=0&LeagueID=00&GameDate=' . $date->toDateString());

        if (!$response->successful()) {
            throw new Exception("Failed to get NBA games from " . $date->toDateString(), 503);
        }
            
        return $response;
    }

    public static function getGameByid(string $gameId)
    {
        $response = Http::withHeaders(self::HEADERS)
            ->get(
                self::BASE_URL 
        ...
        if (!$response->successful()) {
            throw new Exception("Failed to get NBA game by id " . $gameId, 503);
        }
    ...
    {
        $games = self::getGameDayUrl($date)->json('scoreboard.games');
    ...
    public function createManyNbaPlayerScore(array $data, NbaGame $nbaGame, NbaTeam $nbaTeam): void
    {
        ...
        foreach ($data as $playerScore) {
            ...
            $this->nbaPlayerScoreRepository->create([
                ...
                'mins' => $statistics['minutes'] ? $statistics['minutes'] : '00:00',
                'points' => $statistics['points'],
                'rebounds' => $statistics['reboundsTotal'],
                'assists' => $statistics['assists'],
                'steals' => $statistics['steals'],
