...
class NhlMarketService
{
    ...
    {
        $teams = NhlTeam::with('players')->get();
    ...
    public function syncMarkets(?string $marketId = null): void
    {
        ...
        if (empty($gameIds)) {
            Log::warning('Sin gameIds para sincronizar mercados NHL', ['market_id' => $marketId]);
            return;
        }

        $playersByMarketId = NhlPlayer::whereNotNull('market_id')->get()->keyBy('market_id');

        if ($playersByMarketId->isEmpty()) {
            Log::warning('No hay jugadores con market_id asignado para sincronizar mercados NHL');
        }
        ...
        foreach ($gameIds as $targetMarketId) {
            ...
            if (!$game) {
                Log::warning('Juego NHL no encontrado para mercado', ['game_id' => $targetMarketId]);
                continue;
            }
    ...
    private function resolveGameFromTeamPayload(string $marketId, array $teamPayload): ?NhlGame
    {
        $game = NhlGame::where('market_id', $marketId)->first();
        ...
        $homeTeam = $homeMarketId ? NhlTeam::where('market_id', $homeMarketId)->first() : null;
        $awayTeam = $awayMarketId ? NhlTeam::where('market_id', $awayMarketId)->first() : null;

        if (!$homeTeam || !$awayTeam) {
            return null;
        }

        return NhlGame::where('home_team_id', $homeTeam->id)
            ->where('away_team_id', $awayTeam->id)
            ->first();
    }

    private function syncGameMarketData(NhlGame $game, string $marketId, array $teamPayload): void
    {
        $toWin = collect($teamPayload)->firstWhere('statistic', 'to win');
        ...
        NhlGameMarket::updateOrCreate(['game_id' => $game->id], $updates);
    }

    private function syncPlayerMarketData(NhlGame $game, string $marketId, Collection $playersByMarketId): void
    {
